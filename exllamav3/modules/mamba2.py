...
class Mamba2(Module):
    ...
    def forward(
        ...
        # through an internal CUDA graph from the third invocation on. The graph reads the
        # in/out projection trellis weights directly and never sees a runtime LoRA, so fall
        # back to the python path while one is loaded (guard per call: the graph is cached
        # and a LoRA can be attached/detached after build)
        if (
